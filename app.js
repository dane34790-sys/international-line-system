...
function startSplashAnimation(resolve) {
  const messages = [
    "🔹 Initializing System...", "🔹 Loading Modules...",
    "🔹 Connecting to Database...", "🔹 Server Status: ONLINE",
    "🔹 Encryption: ACTIVE", "🔹 International Line System Russia",
    "🔹 System Ready!"
  ];
  let msgIndex = 0, charIndex = 0, progress = 0;

  document.getElementById("app").innerHTML = `
    <div class="splash-screen" style="display:flex;flex-direction:column;justify-content:center;align-items:center;height:100vh;width:100vw;background:#000000;color:#00ff88;font-family:'Courier New',monospace;padding:20px;box-sizing:border-box;position:fixed;top:0;left:0;z-index:99999;">
      <div style="display:flex;gap:3px;margin-bottom:3px;justify-content:center;flex-wrap:wrap;max-width:350px;">
        ${["I","N","T","E","R","N","A","T","I","O","N","A","L"].map((char,i)=>`<div id="charBox_${i}" style="width:22px;height:30px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:bold;background:rgba(0,255,136,0.03);border:1px solid rgba(0,255,136,0.08);border-radius:3px;color:rgba(0,255,136,0.08);transition:all 0.4s ease;font-family:'Courier New',monospace;">${char}</div>`).join('')}
      </div>
      <div style="display:flex;gap:3px;margin-bottom:3px;justify-content:center;flex-wrap:wrap;">
        ${["L","I","N","E"].map((char,i)=>`<div id="charBox_${i+13}" style="width:22px;height:30px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:bold;background:rgba(0,255,136,0.03);border:1px solid rgba(0,255,136,0.08);border-radius:3px;color:rgba(0,255,136,0.08);transition:all 0.4s ease;font-family:'Courier New',monospace;">${char}</div>`).join('')}
      </div>
      <div style="display:flex;gap:3px;margin-bottom:3px;justify-content:center;flex-wrap:wrap;">
        ${["S","Y","S","T","E","M"].map((char,i)=>`<div id="charBox_${i+17}" style="width:22px;height:30px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:bold;background:rgba(0,255,136,0.03);border:1px solid rgba(0,255,136,0.08);border-radius:3px;color:rgba(0,255,136,0.08);transition:all 0.4s ease;font-family:'Courier New',monospace;">${char}</div>`).join('')}
      </div>
    ...
    <style>@keyframes blink{0%,50%{opacity:1}51%,100%{opacity:0}}@keyframes progressGlow{0%{background-position:200% 0}100%{background-position:-200% 0}}.char-active{background:rgba(0,255,136,0.2)!important;border-color:#00ff88!important;color:#00ff88!important;box-shadow:0 0 25px rgba(0,255,136,0.25)!important;transform:scale(1.05)}</style>
  `;
  ...
  function typeMessage() {
    if (msgIndex >= messages.length) { setTimeout(() => { resolve(); }, 2000); return; }
    ...
    charIndex++;
    if (charIndex <= fullText.length) { setTimeout(typeMessage, 70); }
...
function generateOTP() {
  ...
  setTimeout(() => { notif.classList.remove('show'); setTimeout(() => { notif.style.display = 'none'; }, 600); }, 5000);
}
...
function verifyOTP() {
  ...
  if (entered === currentOTP) {
    ...
    showLoadingOverlay();
    setTimeout(() => {
      hideLoadingOverlay(); showWelcomeOverlay();
      setTimeout(() => {
        hideWelcomeOverlay();
        document.getElementById('mainApp').style.display = 'flex';
        if (currentMode === 'admin') { window.isAdmin = true; switchMode('admin'); }
        else { window.isAdmin = false; switchMode('employee'); }
        showCustomAlert('✅ Access Granted', 'INTERNATIONAL LINE SYSTEM');
      }, 3000);
    }, 7000);
  } else { document.getElementById('otpError').style.display = 'block'; }
}
...
function startOTPTimer() {
  ...
  otpTimerInterval = setInterval(() => {
    otpSecondsLeft--;
    document.getElementById('otpTimer').textContent = '⏳ ' + otpSecondsLeft + 's';
    if (otpSecondsLeft <= 0) { clearInterval(otpTimerInterval); document.getElementById('resendBtn').disabled = false; document.getElementById('otpTimer').textContent = ''; }
  }, 1000);
}
...
function showWelcomeOverlay() {
  ...
  el.style.backgroundRepeat = 'no-repeat'; el.style.backgroundColor = '#000';
}
...
async function addEmployee() {
  ...
  try {
    ...
    if (e.code === 'auth/email-already-in-use') { alert("⚠️ User already exists!"); return; }
    console.error(e);
    return;
  }
  ...
  const newEmp = {
    ...
    documents: {
      ...
      stopCPU: false, stopRAM: false, stopNetwork: false,
...
function openLinePage(empId) {
  ...
  overlay.innerHTML = `
    <div style="position:relative;padding:15px;">
      <button onclick="closeLinePage()" style="position:sticky;top:10px;float:right;z-index:10;background:#ff1744;color:white;border:none;padding:10px 20px;border-radius:8px;font-weight:bold;cursor:pointer;">← Back</button>
      ...
      <div style="position:relative;z-index:2;padding:15px 15px 0 15px;">
        <div class="employee-info-card">
          <div class="emp-card-header">
            ...
            <div class="emp-name-title"><div class="emp-fullname"><span class="blink-dot"></span> ${fullName}</div><div class="emp-badge blink">● ONLINE</div></div>
...
function closeLinePage() {
  ...
  if (!window.isAdmin) { document.getElementById('mainApp').style.display = 'flex'; renderAllCards(); }
}
...
async function toggleCPU(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopCPU = !e.documents.stopCPU; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }
async function toggleRAM(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopRAM = !e.documents.stopRAM; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }
async function toggleNetwork(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopNetwork = !e.documents.stopNetwork; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }
async function toggleLogs(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopLogs = !e.documents.stopLogs; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }
async function toggleMovement(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopMovement = !e.documents.stopMovement; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }
async function toggleSignal(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopSignal = !e.documents.stopSignal; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }
async function toggleSignalBar(empId) { const e = employees.find(x => x.id == empId); if (!e) return; e.documents.stopSignalBar = !e.documents.stopSignalBar; await saveEmployeesToDatabase(); closeLinePage(); openLinePage(empId); }

function startServerLoad(emp) {
  const cpu = document.getElementById("cpu"), ram = document.getElementById("ram"), network = document.getElementById("network");
  if (cpu || ram || network) setInterval(() => { if (cpu && !emp.documents.stopCPU) cpu.style.width = (40 + Math.random() * 60) + "%"; if (ram && !emp.documents.stopRAM) ram.style.width = (30 + Math.random() * 60) + "%"; if (network && !emp.documents.stopMovement) network.style.width = (30 + Math.random() * 60) + "%"; }, 1000);
}

function startNetworkStats() {
  const nc = document.getElementById("nodesCount"), l = document.getElementById("latency"), u = document.getElementById("uptime");
  if (nc) setInterval(() => { nc.textContent = 1200 + Math.floor(Math.random() * 400); l.textContent = (20 + Math.floor(Math.random() * 40)) + " ms"; u.textContent = (99.90 + Math.random() * 0.09).toFixed(2) + "%"; }, 800);
}

function startLogs(emp) {
  if (window.logInterval) clearInterval(window.logInterval);
  const logs = ["AUTH SUCCESS","DATABASE VERIFIED","FIREBASE CONNECTED","API RESPONSE 200","TOKEN GENERATED","EMPLOYEE SYNC","NETWORK ACTIVE","SERVER READY","ENCRYPTION ENABLED","BACKUP COMPLETED"];
  const logArea = document.getElementById("logArea"); if (!logArea) return;
  window.logInterval = setInterval(() => { if (emp.documents.stopLogs) return; const div = document.createElement("div"); div.style.margin = "4px 0"; div.innerText = "[" + new Date().toLocaleTimeString("en-GB", { hour12: false }) + "] " + logs[Math.floor(Math.random() * logs.length)]; logArea.appendChild(div); if (logArea.children.length > 18) logArea.removeChild(logArea.firstChild); }, 400);
}

function startSessionTimer() {
  if (window.sessionInterval) clearInterval(window.sessionInterval);
  let s = 0;
  window.sessionInterval = setInterval(() => { s++; const el = document.getElementById("sessionTime"); if (el) { const h = String(Math.floor(s / 3600)).padStart(2, '0'), m = String(Math.floor((s % 3600) / 60)).padStart(2, '0'), sec = String(s % 60).padStart(2, '0'); el.textContent = `${h}:${m}:${sec}`; } }, 1000);
}

function startSignalMonitor(emp) {
  const dbm = document.getElementById("dbmValue"), noise = document.getElementById("noiseValue"), loss = document.getElementById("lossValue"), conn = document.getElementById("connectionValue");
  if (dbm && noise && loss && conn) setInterval(() => { if (emp.documents.stopSignal) return; dbm.textContent = -(40 + Math.floor(Math.random() * 45)) + " dBm"; noise.textContent = -(80 + Math.floor(Math.random() * 15)) + " dBm"; loss.textContent = (Math.random() * 0.5).toFixed(2) + "%"; conn.textContent = "STABLE"; conn.style.color = "#00ff88"; }, 1500);
}

function updateSignalDisplay(stopped) {
  const ss = document.getElementById('signalState'); if (ss) { ss.textContent = stopped ? '📶 SIGNAL STOPPED' : '📶 STRONG SIGNAL SIMCARD'; ss.style.color = stopped ? '#ff5252' : '#00ff88'; }
  const dbm = document.getElementById('dbmValue'); if (dbm) { dbm.textContent = stopped ? '--' : '-42 dBm'; dbm.style.color = stopped ? '#ff5252' : '#00ff88'; }
  const noise = document.getElementById('noiseValue'); if (noise) { noise.textContent = stopped ? '--' : '-92 dBm'; noise.style.color = stopped ? '#ff5252' : '#00ff88'; }
  const loss = document.getElementById('lossValue'); if (loss) { loss.textContent = stopped ? '--' : '0.00%'; loss.style.color = stopped ? '#ff5252' : '#00ff88'; }
  const conn = document.getElementById('connectionValue'); if (conn) { conn.textContent = stopped ? '❌ ERROR SIM' : 'STABLE'; conn.style.color = stopped ? '#ff5252' : '#00ff88'; }
}

function updateSignalBarState(stopped) {
  const sf = document.getElementById('signalFill'), sv = document.getElementById('signalValue'); if (!sf || !sv) return;
  if (stopped) { sf.style.width = '50%'; sf.style.background = '#ff1744'; sf.style.boxShadow = '0 0 20px rgba(255,23,68,0.5)'; sv.textContent = 'STOPPED'; sv.style.color = '#ff5252'; if (window.signalIntervalId) { clearInterval(window.signalIntervalId); window.signalIntervalId = null; } }
  else { sf.style.background = '#00ff88'; sf.style.boxShadow = '0 0 20px rgba(0,255,136,0.3)'; sv.style.color = '#00ff88'; if (!window.signalIntervalId) window.signalIntervalId = setInterval(() => { const f = document.getElementById('signalFill'), v = document.getElementById('signalValue'); if (f && v) { const val = 85 + Math.floor(Math.random() * 16); v.textContent = val + "%"; f.style.width = val + "%"; } }, 1000); }
}

function startRadar() {
  const radar = document.getElementById("radarCanvas"); if (!radar) return;
  const ctx = radar.getContext("2d"); let angle = 0;
  function draw() { const w = radar.width, h = radar.height; ctx.clearRect(0, 0, w, h); const cx = w / 2, cy = h / 2; ctx.strokeStyle = "#00ff88"; ctx.lineWidth = 1; for (let r = 30; r <= 90; r += 20) { ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.stroke(); } ctx.beginPath(); ctx.moveTo(cx, 0); ctx.lineTo(cx, h); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0, cy); ctx.lineTo(w, cy); ctx.stroke(); ctx.strokeStyle = "#00ff88"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + Math.cos(angle) * 90, cy + Math.sin(angle) * 90); ctx.stroke(); ctx.fillStyle = "#00ff88"; for (let i = 0; i < 8; i++) { const a = Math.random() * Math.PI * 2, rr = 15 + Math.random() * 75; ctx.beginPath(); ctx.arc(cx + Math.cos(a) * rr, cy + Math.sin(a) * rr, 2.5, 0, Math.PI * 2); ctx.fill(); } angle += 0.02; requestAnimationFrame(draw); } draw();
  setInterval(() => { const tc = document.getElementById("targetCount"), sp = document.getElementById("signalPower"), ss = document.getElementById("scanStatus"), modes = ["ACTIVE","TRACKING","SCANNING","LOCKED"]; if (tc) tc.textContent = 10 + Math.floor(Math.random() * 15); if (sp) sp.textContent = (90 + Math.floor(Math.random() * 10)) + "%"; if (ss) ss.textContent = modes[Math.floor(Math.random() * modes.length)]; }, 1000);
}

function startSignalChart() {
  const canvas = document.getElementById("signalChart"); if (!canvas) return;
  const ctx = canvas.getContext("2d"), data = []; for (let i = 0; i < 120; i++) data.push(60 + Math.random() * 60);
...
function renderCard(emp, isAdmin) {
  ...
  let adminButtons = '';
  if (isAdmin) adminButtons = `<div class="btn-group"><button class="btn btn-save" onclick="saveEmployee('${emp.id}')">💾 Save</button><button class="btn btn-delete" onclick="deleteEmployee('${emp.id}')">🗑 Delete</button></div>`;
...
function renderAllCards() {
  ...
  if (isAdmin) {
    html += `<button onclick="addEmployee()" style="width:100%;padding:12px;margin-bottom:20px;background:#2196f3;color:#fff;border:none;border-radius:12px;font-weight:bold;font-size:15px;cursor:pointer;">➕ Add Employee</button>`;
  }
  ...
  if (employees.length === 0) {
    ...
    if (isAdmin) {
      employees.forEach(emp => { html += renderCard(emp, isAdmin); });
    } else {
      const empId = currentUser?.emp?.id;
      const emp = employees.find(e => e.id == empId);
      if (emp) html += renderCard(emp, false);
    }
  }

  // ✅ برای هر کارمند دکمه LINE جدا
  if (isAdmin) {
    employees.forEach(emp => {
      const isLocked = emp?.documents?.lineLocked || false;
      html += `
        <div class="card" style="text-align:center;margin-top:10px;background:rgba(0,188,212,0.05);border:1px solid rgba(0,188,212,0.2);">
          <div style="font-size:14px;font-weight:bold;color:#00bcd4;margin-bottom:8px;">📡 LINE - ${emp.id}</div>
          <button onclick="openLinePage('${emp.id}')" style="width:100%;padding:10px;background:#00bcd4;color:#000;border:none;border-radius:8px;font-weight:bold;cursor:pointer;margin-bottom:5px;">📡 OPEN LINE</button>
          <button onclick="toggleLineLock('${emp.id}')" style="width:100%;padding:10px;background:${isLocked?'#ff5252':'#00c853'};color:white;border:none;border-radius:8px;font-weight:bold;cursor:pointer;">${isLocked?'🔒 LOCKED':'🔓 UNLOCKED'}</button>
        </div>
      `;
    });
  } else {
...
function switchMode(mode) {
  currentMode = mode;
  document.getElementById('mainApp').style.display = 'flex';
  const mc = document.getElementById('modeSwitchContainer');
  if (window.isAdmin) mc.style.display = 'block'; else mc.style.display = 'none';
