...
async function loadEmployeesFromDatabase() {
  ...
  if (snapshot.exists()) {
    ...
    employees = Array.isArray(data) ? data : Object.values(data);
  } else {
...
function startSplashAnimation(resolve) {
  ...
  const totalChars = 29; let currentCharIndex = 0; const typingElement = document.getElementById('typingText'), progressBar = document.getElementById('progressBar'), progressText = document.getElementById('progressText');
  function lightUpNextChar() { if (currentCharIndex < totalChars) { const box = document.getElementById(`charBox_${currentCharIndex}`); if (box) box.classList.add('char-active'); currentCharIndex++; setTimeout(lightUpNextChar, 150); } }
  setTimeout(lightUpNextChar, 300);
  function typeMessage() { if (msgIndex >= messages.length) { setTimeout(() => { resolve(); }, 2000); return; } const fullText = messages[msgIndex]; const displayText = fullText.substring(0, charIndex); let fullDisplay = ''; for (let i = 0; i < msgIndex; i++) fullDisplay += messages[i] + '\n'; fullDisplay += displayText; typingElement.innerHTML = `${fullDisplay}<span id="cursor" style="display:inline-block;width:2px;height:16px;background:#00ff88;animation:blink 0.8s infinite;"></span>`; progress = (msgIndex / messages.length) * 100 + (charIndex / fullText.length) * (100 / messages.length); progressBar.style.width = Math.min(progress, 100) + "%"; progressText.textContent = Math.floor(Math.min(progress, 100)) + "%"; charIndex++; if (charIndex <= fullText.length) { setTimeout(typeMessage, 70); } else { msgIndex++; charIndex = 0; setTimeout(typeMessage, 400); } }
...
function handleLogin() {
  ...
  const id = document.getElementById('loginId').value.trim(), pass = document.getElementById('loginPassword').value.trim(), error = document.getElementById('loginError');
...
async function deleteEmployee(empId) {
  ...
  const newList = currentList.filter(e => e.id != empId);
  
  await db.ref("employees").set(newList);
  
  employees = newList;
  renderAllCards();
}
async function toggleLine(empId) { const emp = Array.isArray(employees) ? employees.find(e => e.id == empId) : Object.values(employees).find(e => e.id == empId); if (!emp) return; if (!emp.documents) emp.documents = {}; emp.documents.lineEnabled = !emp.documents.lineEnabled; await saveEmployeesToDatabase(); renderAllCards(); }
async function addEmployee() {
  ...
  const newEmp = {
    id: newId, password, email,
    passport: "", name: "", salary: 0, balance: 0,
    iban: "", cardNumber: "", account: "", expiry: "",
    ccv2: "", zip: "", phone: "", Bank: "", birthDate: "",
    documents: {
      lineEnabled: false, lineLocked: false,
      lineCode: "--------------------", expiryStart: Date.now(),
  ...
  // توی Firebase ذخیره کن
  await db.ref("employees").set(currentList);
...
async function checkLineActivation(empId) {
    ...
    if (remaining <= 0) {
        ...
        if (emp) { 
            ...
            await db.ref("employees/" + empId).update({balance: emp.balance, activationInProgress: false, "documents/lineEnabled": true, "documents/lineLocked": false, "documents/expiryStart": emp.documents.expiryStart});
            if (emp.email) { 
                try { 
                    const nowDate = new Date(), expiryDate = new Date(nowDate.getFullYear() + 5, nowDate.getMonth(), nowDate.getDate());
...
function renderAllCards() {
  ...
  else { if (isAdmin) { employees.forEach(emp => { html += renderCard(emp, isAdmin); }); } else { const empId = currentUser?.emp?.id; const emp = Array.isArray(employees) ? employees.find(e => e.id === empId) : Object.values(employees).find(e => e.id === empId); if (emp) { html += renderCard(emp, false); html += `<div id="activationCountdown" style="margin-top:15px;"></div>`; } } }
...
async function toggleLineLock(empId) { const emp = Array.isArray(employees) ? employees.find(e => e.id == empId) : Object.values(employees).find(e => e.id == empId); if (!emp) return; if (!emp.documents) emp.documents = {}; emp.documents.lineLocked = !emp.documents.lineLocked; await saveEmployeesToDatabase(); renderAllCards(); }
